...
{
    internal class ShootProjectileCommand : Interfaces.ICommand
    {
        ...
        private void FirePlayerProjectile()
        {
            if (game.arrow is Projectile)
            {
                ...
                copy.Position = new Vector2(game.player.Position.X + Constants.LINK_WIDTH/2, 
                    game.player.Position.Y + Constants.LINK_HEIGHT/2);
        ...
        private void FireDifferentEnemyProjectile(ISprite gameProjectile)
        {
            if (gameProjectile is Projectile)
            {
                ...
                string initFiringDirection = GetDirection(game.currEnemy.Velocity, "enemy");
